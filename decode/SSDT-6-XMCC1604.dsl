...
DefinitionBlock ("", "SSDT", 1, "XMCC", "XMCC1604", 0x00001000)
{
    ...
    External (_PR_.PR01, UnknownObj)    // Warning: Unknown object
    External (_PR_.PR01._PPC, IntObj)    // (from opcode)
    External (_PR_.PR02, UnknownObj)    // Warning: Unknown object
    External (_PR_.PR02._PPC, IntObj)    // (from opcode)
    External (_PR_.PR03, UnknownObj)    // Warning: Unknown object
    ...
    Scope (\_SB.PCI0.RP01)
    {
        ...
        Field (PEGB, DWordAcc, NoLock, Preserve)
        {
            Offset (0x04), 
            PCMR,   8, 
            Offset (0xA4), 
    ...
    Scope (\_SB.PCI0.RP01.PXSX)
    {
        ...
        Method (HGON, 0, Serialized)
        {
            ...
            Store (\_SB.PCI0.RP01.PXSX.LTRE, LREN)
            Store (One, CEDR)
        ...
        Method (HGOF, 0, Serialized)
        {
            ...
            Store (Zero, ONOF)
            Store (LCTL, ELCT)
            Store (LREN, \_SB.PCI0.RP01.PXSX.LTRE)
            Store (One, LNKD)
                ...
                Case (0x2A)
                {
                    ...
                    Switch (ToInteger (QUTP))
                    {
                        ...
                        Case (0x02)
                        {
                            ...
                            Store (Zero, VERS)
                            Store (Zero, TGPU)
                            Store (\_SB.PCI0.RP01.PXSX.ECTG (), TGPU)
                            Store (Zero, PDTS)
                            Store (Zero, SFAN)
                            Store (Zero, SKNT)
                            Store (Zero, CPUE)
    ...
    Scope (\_SB.PCI0.RP01.PXSX)
    {
        ...
        {
            Store ("<<< GC6I >>>", Debug)
            \_SB.PCI0.RP01.PXSX.EWGC (One)
            Sleep (0x0A)
            Store (LREN, \_SB.PCI0.RP01.PXSX.LTRE)
            Store (One, LNKD)
            While (LNotEqual (\_SB.PCI0.RP01.PXSX.SGPI (0x80060002), One))
            {
                Sleep (One)
            }
        ...
        Method (GC6O, 0, Serialized)
        {
            Store ("<<< GC6O >>>", Debug)
            Store (Zero, LNKD)
            \_SB.PCI0.RP01.PXSX.SGPO (0x00060000, One)
            While (LNotEqual (\_SB.PCI0.RP01.PXSX.SGPI (0x80060002), Zero))
            {
                Sleep (One)
            }

            \_SB.PCI0.RP01.PXSX.SGPO (0x00060000, Zero)
            While (LLess (LNKS, 0x07))
            {
                Sleep (One)
            }
            ...
            Store (\_SB.PCI0.RP01.PXSX.LTRE, LREN)
            Store (One, CEDR)
            \_SB.PCI0.RP01.PXSX.EWGC (Zero)
            Sleep (0x64)
        }

        Method (GETS, 0, Serialized)
        {
            If (LEqual (\_SB.PCI0.RP01.PXSX.SGPI (0x80060002), Zero))
            {
                Store ("<<< GETS() return 0x1 >>>", Debug)
                Return (One)
            }
            ElseIf (LEqual (\_SB.PCI0.RP01.PXSX.SGPI (0x80060002), One))
            {
